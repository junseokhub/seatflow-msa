 ...
 *
 * event 파라미터를 Object가 아니라 VersionedEvent로 명시했다 — Outbox로 나가는
 * 이벤트는 반드시 자신의 스키마 버전(eventVersion())을 제공해야 한다는 걸
 * 컴파일 타임에 강제하기 위함이다. 이 인터페이스를 구현 안 한 이벤트 record를
 * append()에 넘기면 컴파일 자체가 안 된다 — 런타임 캐스팅 실패보다 훨씬 안전하다.
 */
...
public class OutboxAppender {
     ...
     *                   (예: userId, reservationId — "이 이벤트가 어떤 대상에
     *                   대한 것인지"를 나타내는 값이라 의미가 동일하다)
