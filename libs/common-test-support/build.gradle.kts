...
dependencies {
    /*
     * 버전을 명시하지 않는다. spring-boot-testcontainers가 Spring Boot의 dependency management(BOM)를 통해
     * Testcontainers 전체 버전(1.21.4, Spring Boot 3.5.13 기준)을 이미 관리해주고 있다.
     * 여기서 버전을 직접 못박으면(예: 1.21.3) BOM이 원하는 버전과 충돌해 일부 모듈만 다른 버전으로 끌려가는 문제가 생긴다.
     * jdbc:1.21.3 -> 1.21.4로 강제 승격되면서 testcontainers 코어와 미묘하게 어긋났던 것도 이 충돌의 결과였다.
     */
