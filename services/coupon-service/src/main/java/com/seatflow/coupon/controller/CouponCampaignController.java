 ...
 * MysqlCouponService는 "MySQL만으로 풀면 이렇게 짠다"는 비교 학습 기록으로
 * 남겨둔 것이고 실제로 주입되지 않는다.
 *
 * @Qualifier는 필드가 아니라 생성자 파라미터에 붙여야 확실히 적용된다.
 * @RequiredArgsConstructor(필드에 애노테이션을 붙이는 방식)는 Lombok이 파라미터로
 * 복사하는 과정에서 누락될 수 있어(-parameters 컴파일 옵션 여부에 따라 달라짐),
 * 이 클래스는 생성자를 직접 선언한다.
 */
@RestController
