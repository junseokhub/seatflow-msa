 ...
 * 이 사이에 서버가 죽거나 DB 커넥션이 끊기는 등 예측 못한 장애가 나면, Redis는 발급 완료로 마킹됐는데 MySQL엔 아무 기록도 없는 상태가 영구히 남을 수 있다.
 * 그 사용자는 실제 쿠폰 없이 이미 발급받음으로 계속 거부당한다.
 *
 * 이걸 막기 위해 발급 마킹(issued 키)에 짧은 TTL을 건다. MySQL 저장까지 성공해야 confirmIssued()가 이 TTL을 없애 영구 확정으로 바꾼다.
 ...
 * 재고 자체는 마킹 실패 여부와 무관하게 일단 차감된 채로두고,
 * confirmIssued 실패 시(TTL 만료) 그 재고를 되돌리는 것도 명시적으로 처리한다(아래 releaseExpiredHold 참고)
 * 다만 이건 TTL 만료를 감지하는 별도 수단이 필요해 지금 범위에서는 임시 마킹 동안은 재고가 묶여있다가, confirm 성공 시에만 실제로 확정 소비된다는 정도로 단순화한다.
 */
@Component
...
public class CouponRedisProvider {
    ...
    /**
     * MySQL 저장 성공 후 호출. 임시 마킹(PENDING, TTL 있음)을 영구 확정(CONFIRMED, TTL 없음)으로 바꾼다.
     * 이 호출 자체가 안 오면(MySQL 저장 실패, 서버 다운 등) TTL이 그대로 흘러가 자동으로 마킹이 사라진다.
     * @DynamicPropertySource
        static void properties(DynamicPropertyRegistry registry) {
            MysqlContainerSupport.registerDefaultJpaProperties(registry);
        }
     * confirmIssued를 호출하지 않는 것이 곧 복구 트리거다.
    ...
    public void confirmIssued(Long campaignId, String userId) {
        ...
        if (result == null || result == 0L) {
            ...
            * MySQL엔 저장됐는데 Redis 마킹이 없어진 상태라, 이 사용자가 재시도하면 Redis는 발급 가능으로 보고 다시 발급을 내줄 수 있다.
    ...
    /**
     * MySQL 저장이 예외로 잡아낼 수 있는 방식으로 실패했을 때 명시적으로 호출해 재고와 마킹을 즉시 되돌린다.
     * 예외 없이 실패하는 경우(서버 다운 등)는 이 메서드가 호출될 기회조차 없으므로, 그 경우는 issue()가 건 TTL이 대신 처리한다.
     */
