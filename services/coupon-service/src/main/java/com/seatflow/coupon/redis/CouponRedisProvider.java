 ...
 *
 * Redis 성공과 MySQL 저장 사이에는 시간차가 있다(같은 트랜잭션으로 묶을 수 없다 —
 * 서로 다른 저장소이므로). 이 사이에 서버가 죽거나 DB 커넥션이 끊기는 등 예측
 * 못한 장애가 나면, Redis는 "발급 완료"로 마킹됐는데 MySQL엔 아무 기록도 없는
 * 상태가 영구히 남을 수 있다 — 그 사용자는 실제 쿠폰 없이 "이미 발급받음"으로
 * 계속 거부당한다.
 *
 * 이걸 막기 위해 발급 마킹(issued 키)에 짧은 TTL을 건다. MySQL 저장까지 성공해야
 * confirmIssued()가 이 TTL을 없애 "영구 확정"으로 바꾼다. 그 전에 뭐가 실패하든
 * (예외를 못 잡는 경우까지 포함해서) TTL이 지나면 마킹이 저절로 사라져 재시도가
 * 가능해진다 — 별도 배치나 정합성 점검 없이 Redis 자체 기능만으로 복구된다.
 *
 * 재고(remaining) 카운터는 TTL을 걸지 않는다 — 재고 자체는 마킹 실패 여부와
 * 무관하게 "일단 차감된 채로" 두고, confirmIssued 실패 시(TTL 만료) 그 재고를
 * 되돌리는 것도 명시적으로 처리한다(아래 releaseExpiredHold 참고 — 다만 이건
 * TTL 만료를 감지하는 별도 수단이 필요해 지금 범위에서는 "임시 마킹 동안은
 * 재고가 묶여있다가, confirm 성공 시에만 실제로 확정 소비된다"는 정도로 단순화한다).
 */
@Component
