 ...
 * 트래픽이 크지 않을 때는 이것만으로 충분히 안전하다.
 * RedisCouponService와 나란히 남겨 비교하기 위한 학습 기록이다.
 */
@Slf4j
...
public class DefaultMysqlCouponService implements CouponService {
     ...
     *
     * 1. 재고 원자적 차감(increaseIssuedQuantity)
     * WHERE issuedQuantity < totalQuantity 조건의 UPDATE 자체가 락 역할을 한다. 결과가 0이면 재고 소진.
     * 2. 1인 1매 제약 - DB unique 제약(campaignId, userId)에 맡긴다.
     * 동시에 같은 사용자가 두 번 요청해도(광클) 하나만 성공하고 나머지는 DataIntegrityViolationException.
     *
     * 두 단계 다 "먼저 읽고 판단 후 쓰기" 방식이 아니라 "쓰기 자체가 조건이자 판단"이라는 점이 같다.
     * 9편에서 좌석 정원 원자적 UPDATE, 8편 근처에서 회원가입 1062 처리로 반복해서 써온 원칙이다.
     */
    @Override
    @Transactional
