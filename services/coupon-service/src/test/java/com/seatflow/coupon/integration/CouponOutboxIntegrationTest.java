 ...
 * outbox 테이블 이름/컬럼은 common-outbox-jpa 모듈의 실제 엔티티 정의에 맞춰
 * 확인이 필요하다 — 여기서는 일반적으로 쓰이는 이름(outbox, event_type,
 * status)을 가정했다.
 */
@Testcontainers
@ActiveProfiles("test")
...
class CouponOutboxIntegrationTest implements MysqlContainerSupport, RedisContainerSupport {
    ...
    void issuanceRecordsOutboxEventInSameTransaction() {
        ...
        // event_type 컬럼에는 EventTopic.COUPON_ISSUED의 실제 값("coupon.issued",
        // 소문자+점 표기)이 저장된다 — enum 상수 이름(COUPON_ISSUED)이 아니다.
