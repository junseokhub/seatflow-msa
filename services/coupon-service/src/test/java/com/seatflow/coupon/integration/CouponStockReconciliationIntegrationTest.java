...
/**
 * CouponStockReconciliationScheduler가 Redis 재고 누수를 실제로 찾아 복원하는지 검증한다.
 * 정상 흐름(issueCoupon)이 아니라, MySQL 저장 전에 서버가 죽은 것과 같은 상황을 couponRedisProvider.issue()만 직접 호출해 인위적으로 재현한다.
 * 이러면 Redis는 차감됐는데 MySQL엔 아무 기록도 없는, 정확히 누수 상황이 된다.
 * 스케줄러 자체는 @Scheduled로 자동 실행되게 두지 않고(@EnableScheduling 미적용, 15편 설계 그대로),
 * 테스트에서 reconcile()을 직접 호출해 로직만 검증한다.
 */
@Testcontainers
@ActiveProfiles("test")
...
class CouponStockReconciliationIntegrationTest implements MysqlContainerSupport, RedisContainerSupport {
    ...
    void reconcileRestoresLeakedStock() {
        ...
        // 10개 재고 중 실제로 발급된 건 1개뿐이라는 진실에 맞게, 남은 재고가
        // 9로 정확히 보정된다.
    ...
    void reconcileSkipsCampaignWithoutRedisKey() {
        ...
        // 예외 없이 정상 종료되어야 한다.
        // 이 스케줄러의 책임 범위 밖이라 경고만 남기고 넘어가는 것이 의도된 동작이다.
    ...
    void reconcileDoesNotAutoCorrectWhenMysqlExceedsRedis() {
        ...
        // Redis를 거치지 않고 MySQL에 쿠폰을 직접 추가 삽입
        // 다른 경로로 직접 INSERT된 이례적 상황을 인위적으로 재현한다.
        ...
        // leaked < 0 인 상황이므로 자동 보정(restoreLeakedStock)이 호출되지 않아야 한다.
        // remaining이 reconcile 전후로 그대로여야 한다.
        reconciliationScheduler.reconcile();
