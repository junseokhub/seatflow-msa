 ...
 * (seatflow.coupon.pending-ttl-seconds=2), 운영 TTL(30초)을 그대로 기다리지
 * 않고도 빠르게 검증한다.
 */
@Testcontainers
@ActiveProfiles("test")
...
class CouponTtlIntegrationTest implements MysqlContainerSupport, RedisContainerSupport {
    ...
    void confirmedIssuanceSurvivesTtlExpiry() throws InterruptedException {
        ...
        Thread.sleep(3000);   // TTL(2초) 경과 대기 (실제론 30초라 테스트시 변경후 테스트)

        // 재시도하면 여전히 "이미 발급됨"으로 거부돼야 한다 — confirmIssued가
        // TTL을 없애고 영구 마킹으로 바꿨기 때문이다.
    ...
    void unconfirmedIssuanceExpiresAndAllowsRetry() throws InterruptedException {
        ...
        // confirmIssued를 거치지 않고 issue()만 직접 호출해, "MySQL 저장 전에
        // 서버가 죽은 것"과 동일한 상태를 인위적으로 만든다.
        ...
        Thread.sleep(3000);   // TTL(2초) 경과 대기
