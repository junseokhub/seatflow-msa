 ...
 *
 * 발행 검증: outboxAppender로 적재된 이벤트가 OutboxScheduler(1초 폴링)를 거쳐 진짜 Kafka 토픽에 나가는지,
 * 별도의 KafkaConsumer로 직접 폴링해서 확인한다.
 *
 * 구독 검증: 이 테스트가 "다른 서비스인 척" 진짜 KafkaTemplate으로 이벤트를 발행하고,
 * reservation-service의 진짜 @KafkaListener가 그걸 받아서 DB 상태를 정확히 바꾸는지 확인한다.
 *
 * seat-service, payment-service는 실제로 띄우지 않는다.
 * 그 서비스들의 컨슈머/발행 로직은 각자의 단위 테스트에서 이미 검증했고,
 * 여기서는 reservation-service의 프로듀서/컨슈머가 진짜 Kafka와 올바르게 통신하는지만 본다.
 */
@Testcontainers
@ActiveProfiles("test")
...
class SagaKafkaIntegrationTest implements MysqlContainerSupport, KafkaContainerSupport {
    ...
    void setUp() {
        ...
        // OutboxScheduler가 진짜 발행한 걸 직접 확인하기 위한,
        // reservation-service의 진짜 컨슈머 그룹과 무관한 별도 컨슈머(그룹 매번 새로 만들어서 항상 처음부터 읽음).
