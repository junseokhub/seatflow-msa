...
class SeatReleasedEventConsumerTest {

    @Mock
    private CancelSagaOrchestrator orchestrator;

    private SeatReleasedEventConsumer consumer;

    @BeforeEach
    void setUp() {
        ObjectMapper realObjectMapper = new ObjectMapper().findAndRegisterModules();
        consumer = new SeatReleasedEventConsumer(orchestrator, realObjectMapper);
    }

    @Test
    @DisplayName("정상 메시지는 파싱되어 onSeatReleased에 위임된다")
    void validMessageDelegatesToOnSeatReleased() {
        String validMessage = """
        ...
        verify(orchestrator).onSeatReleased(1L, 100L);
    }
    ...
    void malformedMessageThrowsForDlq() {
        ...
        verify(orchestrator, never()).onSeatReleased(anyLong(), anyLong());
    }
}