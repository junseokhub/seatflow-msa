...
public class SeatRedisProvider {
     ...
     * 예전엔 이 메서드가 중괄호 없는 별도 키 형식을 직접 조합해서, holdAll 등이 보는 키와 서로 다른 좌석을 가리키는 버그가 있었다.
     */
     ...
     * 모든 메서드(hold, release, isHeld, holdAll, getHolder, releaseIfOwner)가 반드시 이 메서드를 통해서만 키를 만들어야 한다.
     * 직접 문자열을 조합하면 이 형식(중괄호 해시태그 포함)과 어긋나는 별도의 키가 생긴다.
     */
