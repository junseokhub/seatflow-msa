...
/**
 * show-service 전용 Outbox 발행 스케줄러. MongoOutboxStore에 직접 결합한다.
 * common-outbox 모듈 없이 단독으로 동작하며(다른 DB 구현으로 교체될 일이 없으므로
 * 인터페이스 추상화는 두지 않는다), 애플리케이션의 @EnableScheduling(컴포넌트 스캔)으로
 * 자동 등록된다.
 *
...
public class OutboxScheduler {
    ...
    /** 한 번의 cleanup 사이클에서 삭제할 최대 건수(대량 삭제의 락·복제지연 방지용 상한). */
