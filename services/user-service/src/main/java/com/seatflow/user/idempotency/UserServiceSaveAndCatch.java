 ...
 * DataIntegrityViolationException이 발생하는 순간 그 트랜잭션 전체가 rollback-only로 마킹된다.
 * catch로 예외를 잡아도 이미 마킹된 트랜잭션은 되돌릴 수 없어, 이후 commit 시도 시 UnexpectedRollbackException이 터진다.
 * 컨슈머 쪽에 트랜잭션이 전혀 없을 때만(그리고 앞으로도 안 생긴다는 보장이 있을 때만) 안전하다.
 * 이 조건부 안전성이 정식 채택하지 않은 이유다.
 */
@Slf4j
...
public class UserServiceSaveAndCatch {
    ...
    public void createUser(String userId, String email, String name) {
        try {
            ...
            // 주의
            // 이 catch 블록 자체는 정상 작동하지만, 호출자 트랜잭션이 이미 rollback-only로 마킹된 상태라면 이 메서드가 조용히 끝나도 바깥 트랜잭션의 commit이 실패한다. 이게 [1] 방식의 함정이다.
        }
    }
}