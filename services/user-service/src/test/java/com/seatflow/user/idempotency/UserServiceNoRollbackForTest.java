 ...
 *
 * 중요
 * 이 테스트가 통과한다고 해서 이 방식이 안전하다는 뜻이 아니다.
 * Mock은 saveAndFlush()가 예외를 던진다까지만 흉내 낼 뿐,
 * 그 예외가 나는 순간 진짜 Spring 트랜잭션 매니저가 트랜잭션을 rollback-only로 마킹하는 것까지는 재현하지 못한다.
 * 실제로 이 방식은 진짜 동시 요청 상황에서 UnexpectedRollbackException을 냈고(운영 UserService가 이 방식이었을 때 발견),
 * 그 문제는 이 단위 테스트가 전부 통과한 채로도 숨어 있었다.
 *
 * 진짜 안전성 검증은 IdempotencyStrategyComparisonIntegrationTest의 noRollbackForFailsUnderRealConcurrency(진짜 DB + 진짜 트랜잭션)에서 이뤄진다.
 * 이 클래스는 그 실패를 재현하는 목적으로 일부러 남겨져 있다.
 */
...
class UserServiceNoRollbackForTest {
    ...
    void silentlyCatchesUniqueConstraintViolation() {
        ...
        /**
         * Mock 레벨에서는 이렇게 잘 잡아서 넘기는 것처럼 보인다.
         * 하지만 진짜 Spring 트랜잭션 매니저가 없는 이 테스트 환경에서는,
         * catch 이전에 트랜잭션이 rollback-only로 마킹되는 부작용 자체가 시뮬레이션되지 않는다.
         * 그래서 이 테스트는 통과하지만 운영에서는 문제가 났었다.
         */
        assertThatCode(() -> service.createUser("user-1", "test@example.com", "테스트"))
                .doesNotThrowAnyException();
    }
